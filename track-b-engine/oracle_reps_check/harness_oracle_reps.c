...
#define TRIM_PCT           5
#ifndef REPS
#define REPS               100   /* memcmps per timed sample — amplifies the
                                  * sub-ns early-exit signal above the macOS
                                  * clock_gettime granularity (~24ns).
                                  * Override at build time: -DREPS=2000 */
#endif
...
int main(int argc, char *argv[]) {
    ...
    /* Interleaved measurement — REPS memcmps per timed sample so the amplified
     * interval (~REPS * signal) clears the macOS clock_gettime granularity. */
    for (int i = 0; i < runs; i++) {
        uint64_t t0;
        int r;
        t0 = now_ns();
        for (r = 0; r < REPS; r++) sink += patched_compare(buf_A, buf_A_match, MLDSA_CTILDEBYTES);
        sa[i] = now_ns() - t0;
        t0 = now_ns();
        for (r = 0; r < REPS; r++) sink += patched_compare(buf_B, buf_B_mismatch, MLDSA_CTILDEBYTES);
        sb[i] = now_ns() - t0;
    }
    (void)sink;
