...
- What: Installed AFL++ 4.09c in WSL2; built 4 weakened Kyber512 fuzz targets (leak2, leak4,
  ...
  runs for each target via setsid + AFL_NO_UI=1 + AFL_I_DONT_CARE_ABOUT_MISSING_CRASHES=1.
  Fixed: (1) missing #include <unistd.h> in harness_kyber.c (afl-clang-fast strict ISO C99);
  (2) NTFS /mnt/d slowdown — findings/corpus dirs moved to WSL ext4 ~/fuzz/<target>/ for
  ~1800 exec/s vs 0.5 exec/s on NTFS. start_fuzz_all.sh script added to fuzzing/ for
  repeatable relaunch.
- Settings: afl-fuzz -V 86400, persistent mode, ASAN, CLOCK_MONOTONIC_RAW seed (768B zeroed
  ciphertext), 4 parallel instances (one per target), WSL2/Ubuntu, 18 cores available.
  Exec rate: ~107k execs/min per target at 60s mark (1,780 exec/s).
- Result: All 4 processes confirmed running (pgrep -c afl-fuzz = 4). Findings in ~/fuzz/<t>/
  on WSL ext4; summary JSON written to shared/findings/ by summarize_afl.py at run end.
  Logs at /tmp/fuzz_logs/fuzz_<target>.log.
- Takeaway: 24h run in progress. Results available ~2026-06-19. Compare unique_paths /
  time-to-first-path / crashes against LLM adversary loop rediscoveries per target.
