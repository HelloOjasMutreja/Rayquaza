from bootstrap.wizard import _stage_for_line, _style_line
...
def test_style_line_highlights_promoted_green_bold():
    ...
    assert result.startswith("[bold green]")
    assert result.endswith("[/bold green]")


def test_style_line_highlights_demoted_yellow():
    result = _style_line("[Cycle 1] Hypothesis H002 → DEMOTED (t=0.4, sig=False)")
    assert result.startswith("[yellow]")


def test_style_line_highlights_invalidated_yellow():
...
def test_style_line_highlights_warning():
    ...
    assert result.startswith("[bold yellow]")
...
def test_style_line_dims_waiting_lines():
    ...
    assert result.startswith("[dim cyan]")
