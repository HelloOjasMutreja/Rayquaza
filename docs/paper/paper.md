...
Yang et al. [REF-AUTOAUDIT] apply LLMs to smart-contract security auditing, demonstrating that domain-specific fine-tuning significantly improves vulnerability detection rates over general-purpose models. Their work highlights the importance of domain alignment — a principle that motivates our prompt engineering for PQC-specific vulnerability classes (§3.3).
...
AFL++ and its derivatives are the dominant automated vulnerability discovery tools for C code. For cryptographic implementations, coverage-guided fuzzing faces a structural challenge: side-channel leaks are not observable as crashes, sanitizer violations, or coverage differences — the information is in *timing*, not *path structure*. Differential fuzzing approaches such as DIFFUZZ [REF-MAZE] extend AFL++ to maximise resource-usage differences between two program copies executing under related inputs, making side-channel differences observable to the fuzzer. However, even differential fuzzing requires the fuzzer to *observe* the timing difference as a quantified resource cost, which standard AFL++ instrumentation does not provide.
...
- [REF-MAZE] **[CITATION NEEDS VERIFICATION]** Referenced in §2.4 as "taint-guided and differential fuzzing approaches" extending AFL++ towards secret-dependent paths. The placeholder "Wang et al., MAZE: Towards Automated Heap Feng Shui, 2021" refers to a heap-exploitation paper and does not match this description. The most likely intended reference is: S. Nilizadeh, Y. Noller, and C. Pasareanu, "DIFFUZZ: Differential Fuzzing for Side-Channel Analysis," ICSE 2019, arXiv: https://arxiv.org/abs/1811.07005. Replace before submission.
